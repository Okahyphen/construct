...
Collection = require('base'):derive(function (_, self, ...)
	local data = { ... }
...
function Collection.fn:length ()
	return #self.data
end

function Collection.fn:nth (index, value)
...
function Collection.fn:each (operation)
...
function Collection.fn:has (value, start, final)
...
function Collection.fn:indices (target)
	local results = {}
	local insert = 1
...
function Collection.fn:all (test)
	for index = 1, self:length() do
...
function Collection.fn:any (test)
	for index = 1, self:length () do
...
function Collection.fn:reduce (operation, initial)
...
function Collection.fn:count (test)
	local result = 0
...
function Collection.fn:join (separator)
...
function Collection.fn:unpack (from, to)
